...
def test_vqvae(model_path, test_loader, indices=None, max_images=8):
    ...
    vqvae = VQVAE(in_channels=1, vocab_size=8192, z_channels=64, test_mode=True).to(device)
    ...
    avg_dice = total_dice / len(selected_images)
    print(f"Test Dice Score: {avg_dice:.4f}")

    # Display the images in an 8x2 grid
    display_results(selected_images, save_path="test_results1.png")
...
if __name__ == "__main__":
    ...
    test_indices = [0, 5, 10, 15, 20, 25, 30, 40]  # Example indices (max 8)

    test_vqvae("./checkpoints/vqvae_best.pth", test_loader, indices=test_indices, max_images=8)

...
def train_vqvae(vqvae, train_loader, val_loader, num_epochs=50, save_path="./checkpoints"):
    ...
    for epoch in range(num_epochs):
        ...
        for img in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs} [Train]"):
            ...
            loss = dice_loss(rec_img, img) + perceptual_loss(rec_img, img) + 0.5 * vq_loss
            loss.backward()
            optimizer.step()

            total_loss += loss.item()
            total_dice += dice_score(rec_img, img)

        writer.add_scalar("Loss/Train", total_loss / len(train_loader), epoch)
        writer.add_scalar("Dice/Train", total_dice / len(train_loader), epoch)
        ...
        with torch.no_grad():
            for img in tqdm(val_loader, desc=f"Epoch {epoch+1}/{num_epochs} [Validation]"):
                ...
                val_loss += loss.item()
                val_dice += dice_score(rec_img, img)

        avg_val_loss = val_loss / len(val_loader)
        writer.add_scalar("Loss/Validation", avg_val_loss, epoch)
        writer.add_scalar("Dice/Validation", val_dice / len(val_loader), epoch)
        ...
        print(f"Epoch {epoch+1}, Train Loss: {total_loss/len(train_loader):.4f}, Dice: {total_dice/len(train_loader):.4f}, "
              f"Val Loss: {avg_val_loss:.4f}, Val Dice: {val_dice/len(val_loader):.4f}")
...
if __name__ == "__main__":
    ...
    train_dataset = CustomImageDataset("/media/viplab/DATADRIVE1/skin_lesion/ISIC2018/Training_GroundTruth/", transform=transform)
    val_dataset = CustomImageDataset("/media/viplab/DATADRIVE1/skin_lesion/ISIC2018/Validation_GroundTruth/", transform=transform)
    ...
    vqvae = VQVAE(in_channels=1, vocab_size=8192, z_channels=64, test_mode=False)
    train_vqvae(vqvae, train_loader, val_loader)